...
pub fn handle_input(currstate: &state::TextInputState, key: &KeyInfo) -> state::TextInputState {
    ...
    match key.modifiers {
        KeyModifiers::NONE => match key.code {
            ...
            KeyCode::Right => {
                let new_cursor_pos = if cursor_pos == contents.chars().count() {
                    cursor_pos
                } else {
                    cursor_pos + 1
                };
            ...
            KeyCode::End => state::TextInputState {
                cursor_pos: contents.chars().count(),
                ..currstate.clone()
            },
            KeyCode::Home => state::TextInputState {
                cursor_pos: 0,
                ..currstate.clone()
            },
            _ => currstate.clone(),
        ...
        KeyModifiers::CONTROL => match key.code {
            KeyCode::Backspace | KeyCode::Char('w') => {
                ...
                let new_cursor_pos =
                    cursor_pos.saturating_sub(contents.len() - res_string.chars().count());
                state::TextInputState {
                    contents: res_string,
                    cursor_pos: new_cursor_pos,
                }
...
pub fn render(
    label: &str,
    currstate: &state::TextInputState,
    styles: &Styles,
    mode: &InputMode,
) -> io::Result<()> {
    ...
    if styles.active {
        let char_under_cursor = currstate
            .contents
            .chars()
            .nth(currstate.cursor_pos)
            .unwrap_or('_')
            .to_string();
        queue!(
            stdout,
            cursor::MoveTo(
                styles.margin_left + label_width + currstate.cursor_pos as u16,
                styles.margin_top
            ),
...
fn render_multiline(
    ...
    for local_y in 0..(region_height) {
        for local_x in 0..(region_width) {
            ...
            let render_cursor = current_char_index == currstate.cursor_pos as u16 && styles.active;
            queue!(
                stdout,
